...
#define CERTF "./cert_mkcert.pem"
//#define CERTF "./crt.pem"
#define KEYF "./key_mkcert.pem"
//#define KEYF "./key.pem"
...
static char* read_request_line(struct HTTPRequest *req, FILE *in);
static struct HTTPHeaderField* read_header_field(FILE *in, char*);
...
static void ssl_wrapped_sockeet(int );
SSL_CTX *ctx;
SSL     *ssl;
...
#define USAGE "Usage: %s [--port=n] [--enc] <docroot>\n"
...
static struct option longopts[] = {
    ...
    //{"enc",  no_argument,       &encrypt_mode, 1},
...
void 
ssl_wrapped_sockeet(int socket)
{
    ssl = SSL_new(ctx);
    if( ssl == NULL)
    {
        log_exit("failed to create ssl session");
    }
...
read_request(FILE *in)
{
    ...
    p = read_request_line(req, in);
    req->header = NULL;
    while (h = read_header_field(in,p)) {
        h->next = req->header;
        req->header = h;
    }
...
read_request_line(struct HTTPRequest *req, FILE *in)
{
    ...
    // SSL_read(ssl, buf, LINE_BUF_SIZE)
    if( encrypt_mode)
    {
        int n = SSL_read(ssl, buf, LINE_BUF_SIZE);
        buf[n] = '\0';
    }
    else if (!fgets(buf, LINE_BUF_SIZE, in))
...
read_header_field(FILE *in, char *req_buf)
{
    struct HTTPHeaderField *h;
    char buf[LINE_BUF_SIZE];
    char *p;
    ...
    if( encrypt_mode )
    {
        strcpy(buf,(req_buf+2)); // 이전에 read 해둔 버퍼 포인터 넘겨줌
    }
    else if (!fgets(buf, LINE_BUF_SIZE, in))
...
do_file_response(struct HTTPRequest *req, FILE *out, char *docroot)
{
    ...
    {
        int buf_len;
        char tmpbuf[LINE_BUF_SIZE];

        buf_len = sprintf(tmpbuf, "Content-Length: %ld\r\n", info->size);
        SSL_write(ssl, tmpbuf, buf_len);
        buf_len = sprintf(tmpbuf, "Content-Type: %s\r\n", guess_content_type(info));
        SSL_write(ssl, tmpbuf, buf_len);
    ...
    if (strcmp(req->method, "HEAD") != 0) {
        ...
        for (;;) {
            ...
            if( encrypt_mode )
            {
                SSL_write(ssl, buf, BLOCK_BUF_SIZE);
            }
            else
            {
...
method_not_allowed(struct HTTPRequest *req, FILE *out)
{
    ...
    if( encrypt_mode )
    {
        buf_len = sprintf(tmpbuf, "Content-Type: text/html\r\n"
                                "\r\n"
                                "<html>\r\n"
                                "<header>\r\n"
                                ...
                                "<header>\r\n"
                                "<body>\r\n"
                                "<p>The request method %s is not implemented</p>\r\n"
                                "</body>\r\n"
                                "</html>\r\n", req->method);
        SSL_write(ssl, tmpbuf, buf_len);
    }
...
output_common_header_fields(struct HTTPRequest *req, FILE *out, char *status)
{
    ...
    if( encrypt_mode )
    {
        buf_len = sprintf(tmpbuf, "HTTP/1.%d %s\r\n", HTTP_MINOR_VERSION, status);
        SSL_write(ssl, tmpbuf, buf_len);
        buf_len = sprintf(tmpbuf, "Date: %s\r\n", buf);
        SSL_write(ssl, tmpbuf, buf_len);
        buf_len = sprintf(tmpbuf, "Server: %s/%s\r\n", SERVER_NAME, SERVER_VERSION);
        SSL_write(ssl, tmpbuf, buf_len);
        buf_len = sprintf(tmpbuf, "Connection: close\r\n");
        SSL_write(ssl, tmpbuf, buf_len);
    }
    else
    {
